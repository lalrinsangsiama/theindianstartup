...
export default function P2LessonPage() {
  ...
  useEffect(() => {
    const fetchLesson = async () => {
      try {
        const response = await fetch(`/api/products/P2/lessons/${lessonId}`, {
          credentials: 'include'
        });
        ...
        logger.error('Error fetching P2 lesson:', error);
      } finally {
        setLoading(false);
      }
    };
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (nextDay <= totalDays) {
          router.push(`/products/p2/lessons/${nextDay}`);
        } else {
          router.push('/products/p2');
        }
      } else {
  ...
  const navigateToDay = (day: number) => {
    router.push(`/products/p2/lessons/${day}`);
  };
      ...
      <DashboardLayout>
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <Button
              variant="outline"
              onClick={() => router.push('/products/p2')}
          ...
          <Card className="mb-6">
            <div className="p-8">
              ...
              <ActivityCapture
                activityTypeId={`p2_day_${lesson.day}_activity`}
                activityName={`Day ${lesson.day} Incorporation Activity`}
                lessonId={lesson.id}
                courseCode="P2"
                moduleId={`p2_mod_${Math.ceil(lesson.day / 4)}`}
              />
            </div>
          </Card>
