...
export default function P12LessonPage() {
  ...
  useEffect(() => {
    const fetchLesson = async () => {
      try {
        const response = await fetch(`/api/products/p12/lessons/${lessonId}`, {
          credentials: 'include'
        });
  ...
  const completeLesson = async () => {
    ...
    setCompleting(true);
    try {
      const response = await fetch(`/api/products/p12/lessons/${lessonId}/complete`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (nextDay <= 60) {
          router.push(`/products/p12/lessons/${nextDay}`);
        } else {
  ...
  const navigateToDay = (day: number) => {
    router.push(`/products/p12/lessons/${day}`);
  };
