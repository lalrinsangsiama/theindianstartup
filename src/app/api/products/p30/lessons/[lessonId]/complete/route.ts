...
export async function POST(
  ...
  try {
    ...
    // Check if user has access to P30
    const access = await checkProductAccess(user.id, 'P30');
    if (!access.hasAccess) {
      return NextResponse.json(
        { error: 'Access denied. Please purchase P30 Crisis Management & Business Continuity course.' },
        { status: 403 }
      );
    }
    ...
    const { data: lesson, error: lessonError } = await supabase
      ...
      `)
      .eq('day', lessonDay)
      .eq('module.product.code', 'P30')
      .single();

    if (lessonError || !lesson) {
      logger.error('Error fetching P30 lesson for completion:', lessonError);
      return NextResponse.json(
    ...
    const { data: purchase, error: purchaseError } = await supabase
      ...
      .or(`productCode.eq.P30,productCode.eq.ALL_ACCESS`)
      .limit(1)
      .single();
    ...
    logger.info(`P30 Lesson ${lessonDay} completed by user ${user.id}`);
    ...
    logger.error('Error in P30 lesson completion API:', error);
    return NextResponse.json(
