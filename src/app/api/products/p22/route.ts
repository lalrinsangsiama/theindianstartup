...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get P22 product details with modules and lessons
    ...
    if (productError) {
      logger.error('Error fetching P22 product:', productError);
    ...
    if (user) {
      ...
      if (hasAccess && product?.modules) {
        // Get user's progress for P22 lessons
    ...
    logger.error('Error in P22 product API:', error);
