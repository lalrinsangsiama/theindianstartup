...
export default function P21LessonPage() {
  ...
  useEffect(() => {
    const fetchLesson = async () => {
      try {
        const response = await fetch(`/api/products/p21/lessons/${lessonId}`, {
          credentials: 'include'
        });
        ...
        logger.error('Error fetching P21 lesson:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const completeLesson = async () => {
    ...
    setCompleting(true);
    try {
      const response = await fetch(`/api/products/p21/lessons/${lessonId}/complete`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (nextDay <= totalDays) {
          router.push(`/products/p21/lessons/${nextDay}`);
        } else {
          router.push('/products/p21');
        }
      } else {
  ...
  const navigateToDay = (day: number) => {
    router.push(`/products/p21/lessons/${day}`);
  };

  if (loading) {
    return (
      <ProductProtectedRoute productCode="P21">
        <DashboardLayout>
      ...
      <DashboardLayout>
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <Button
              variant="outline"
              onClick={() => router.push('/products/p21')}
          ...
          <Card className="mb-6">
            <div className="p-8">
              ...
              {lesson.day <= 25 && (
                <ActivityCapture
                  activityTypeId={`p21_day_${lesson.day}_activity`}
                  activityName={`Day ${lesson.day} Startup Acquisition Activity`}
                  lessonId={lesson.id}
                  courseCode="P21"
                  moduleId={`p21_mod_${Math.ceil(lesson.day / 5)}`}
                />
              )}
            </div>
          </Card>
