...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get P19 product details with modules and lessons
    ...
    if (productError) {
      logger.error('Error fetching P19 product:', productError);
    ...
    if (user) {
      ...
      if (hasAccess && product?.modules) {
        // Get user's progress for P19 lessons
    ...
    logger.error('Error in P19 product API:', error);
