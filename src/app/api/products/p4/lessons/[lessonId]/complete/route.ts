...
export async function POST(
  ...
  try {
    ...
    // Check if user has access to P4
    const access = await checkProductAccess(user.id, 'P4');
    if (!access.hasAccess) {
      return NextResponse.json(
        { error: 'Access denied. Please purchase P4 Finance Stack - CFO-Level Mastery course.' },
        { status: 403 }
      );
    }
    ...
    const { data: lesson, error: lessonError } = await supabase
      ...
      `)
      .eq('day', lessonDay)
      .eq('module.product.code', 'P4')
      .single();

    if (lessonError || !lesson) {
      logger.error('Error fetching P4 lesson for completion:', lessonError);
      return NextResponse.json(
    ...
    const { data: purchase, error: purchaseError } = await supabase
      ...
      .or(`productCode.eq.P4,productCode.eq.ALL_ACCESS`)
      .limit(1)
      .single();
    ...
    logger.info(`P4 Lesson ${lessonDay} completed by user ${user.id}`);
    ...
    logger.error('Error in P4 lesson completion API:', error);
    return NextResponse.json(
