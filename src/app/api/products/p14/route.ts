...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get P14 product details with modules and lessons
    ...
    if (productError) {
      logger.error('Error fetching P14 product:', productError);
    ...
    if (user) {
      ...
      if (hasAccess && product?.modules) {
        // Get user's progress for P14 lessons
    ...
    logger.error('Error in P14 product API:', error);
