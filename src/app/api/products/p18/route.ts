...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get P18 product details with modules and lessons
    ...
    if (productError) {
      logger.error('Error fetching P18 product:', productError);
    ...
    if (user) {
      ...
      if (hasAccess && product?.modules) {
        // Get user's progress for P18 lessons
    ...
    logger.error('Error in P18 product API:', error);
