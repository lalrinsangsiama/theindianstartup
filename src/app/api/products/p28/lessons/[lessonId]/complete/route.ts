...
export async function POST(
  ...
  try {
    ...
    // Check if user has access to P28
    const access = await checkProductAccess(user.id, 'P28');
    if (!access.hasAccess) {
      return NextResponse.json(
        { error: 'Access denied. Please purchase P28 AI & Machine Learning for Startups course.' },
        { status: 403 }
      );
    }
    ...
    const { data: lesson, error: lessonError } = await supabase
      ...
      `)
      .eq('day', lessonDay)
      .eq('module.product.code', 'P28')
      .single();

    if (lessonError || !lesson) {
      logger.error('Error fetching P28 lesson for completion:', lessonError);
      return NextResponse.json(
    ...
    const { data: purchase, error: purchaseError } = await supabase
      ...
      .or(`productCode.eq.P28,productCode.eq.ALL_ACCESS,productCode.eq.DEEP_TECH`)
      .limit(1)
      .single();
    ...
    logger.info(`P28 Lesson ${lessonDay} completed by user ${user.id}`);
    ...
    logger.error('Error in P28 lesson completion API:', error);
    return NextResponse.json(
