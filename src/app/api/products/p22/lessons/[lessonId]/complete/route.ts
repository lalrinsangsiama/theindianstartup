...
export async function POST(
  ...
  try {
    ...
    // Check if user has access to P22
    const access = await checkProductAccess(user.id, 'P22');
    if (!access.hasAccess) {
      return NextResponse.json(
        { error: 'Access denied. Please purchase P22 course.' },
        { status: 403 }
      );
    }
    ...
    const { data: lesson, error: lessonError } = await supabase
      ...
      `)
      .eq('day', lessonDay)
      .eq('module.product.code', 'P22')
      .single();

    if (lessonError || !lesson) {
      logger.error('Error fetching P22 lesson for completion:', lessonError);
      return NextResponse.json(
    ...
    const { data: purchase, error: purchaseError } = await supabase
      ...
      .or(`productCode.eq.P22,productCode.eq.ALL_ACCESS,productCode.eq.ECOMMERCE_COMPLETE`)
      .limit(1)
      .single();
    ...
    logger.info(`P22 Lesson ${lessonDay} completed by user ${user.id}`);
    ...
    logger.error('Error in P22 lesson completion API:', error);
    return NextResponse.json(
