...
export default function P28LessonPage() {
  ...
  useEffect(() => {
    const fetchLesson = async () => {
      try {
        const response = await fetch(`/api/products/p28/lessons/${lessonId}`, {
          credentials: 'include'
        });
        ...
        logger.error('Error fetching P28 lesson:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const completeLesson = async () => {
    ...
    setCompleting(true);
    try {
      const response = await fetch(`/api/products/p28/lessons/${lessonId}/complete`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (nextDay <= totalDays) {
          router.push(`/products/p28/lessons/${nextDay}`);
        } else {
          router.push('/products/p28');
        }
      } else {
  ...
  const navigateToDay = (day: number) => {
    router.push(`/products/p28/lessons/${day}`);
  };

  if (loading) {
    return (
      <ProductProtectedRoute productCode="P28">
        <DashboardLayout>
      ...
      <DashboardLayout>
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <Button
              variant="outline"
              onClick={() => router.push('/products/p28')}
          ...
          <Card className="mb-6">
            <div className="p-8">
              ...
              {lesson.day <= 25 && (
                <ActivityCapture
                  activityTypeId={`p28_day_${lesson.day}_activity`}
                  activityName={`Day ${lesson.day} Activity`}
                  lessonId={lesson.id}
                  courseCode="P28"
                  moduleId={`p28_mod_${Math.ceil(lesson.day / 5)}`}
                />
              )}
            </div>
          </Card>
