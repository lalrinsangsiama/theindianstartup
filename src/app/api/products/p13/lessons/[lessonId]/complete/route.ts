...
export async function POST(
  ...
  try {
    ...
    // Check if user has access to P13
    const access = await checkProductAccess(user.id, 'P13');
    if (!access.hasAccess) {
      return NextResponse.json(
        { error: 'Access denied. Please purchase P13 Food Processing Mastery course.' },
        { status: 403 }
      );
    }
    ...
    const { data: lesson, error: lessonError } = await supabase
      ...
      `)
      .eq('day', lessonDay)
      .eq('module.product.code', 'P13')
      .single();

    if (lessonError || !lesson) {
      logger.error('Error fetching P13 lesson for completion:', lessonError);
      return NextResponse.json(
    ...
    const { data: purchase, error: purchaseError } = await supabase
      ...
      .or(`productCode.eq.P13,productCode.eq.ALL_ACCESS,productCode.eq.SECTOR_MASTERY`)
      .limit(1)
      .single();
    ...
    logger.info(`P13 Lesson ${lessonDay} completed by user ${user.id}`);
    ...
    logger.error('Error in P13 lesson completion API:', error);
    return NextResponse.json(
