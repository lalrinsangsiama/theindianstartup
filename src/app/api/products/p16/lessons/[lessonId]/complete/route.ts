...
export async function POST(
  ...
  try {
    ...
    // Check if user has access to P16
    const access = await checkProductAccess(user.id, 'P16');
    if (!access.hasAccess) {
      return NextResponse.json(
        { error: 'Access denied. Please purchase P16 HR & Team Building Mastery course.' },
        { status: 403 }
      );
    }
    ...
    const { data: lesson, error: lessonError } = await supabase
      ...
      `)
      .eq('day', lessonDay)
      .eq('module.product.code', 'P16')
      .single();

    if (lessonError || !lesson) {
      logger.error('Error fetching P16 lesson for completion:', lessonError);
      return NextResponse.json(
    ...
    const { data: purchase, error: purchaseError } = await supabase
      ...
      .or(`productCode.eq.P16,productCode.eq.ALL_ACCESS,productCode.eq.CORE_FUNCTIONS`)
      .limit(1)
      .single();
    ...
    logger.info(`P16 Lesson ${lessonDay} completed by user ${user.id}`);
    ...
    logger.error('Error in P16 lesson completion API:', error);
    return NextResponse.json(
