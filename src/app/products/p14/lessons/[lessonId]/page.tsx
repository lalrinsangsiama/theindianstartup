...
export default function P14LessonPage() {
  ...
  useEffect(() => {
    const fetchLesson = async () => {
      try {
        const response = await fetch(`/api/products/p14/lessons/${lessonId}`, {
          credentials: 'include'
        });
        ...
        logger.error('Error fetching P14 lesson:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const completeLesson = async () => {
    ...
    setCompleting(true);
    try {
      const response = await fetch(`/api/products/p14/lessons/${lessonId}/complete`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        if (nextDay <= totalDays) {
          router.push(`/products/p14/lessons/${nextDay}`);
        } else {
          router.push('/products/p14');
        }
      }
    } catch (error) {
  ...
  const navigateToDay = (day: number) => {
    router.push(`/products/p14/lessons/${day}`);
  };

  if (loading) {
    return (
      <ProductProtectedRoute productCode="P14">
        <DashboardLayout>
      ...
      <DashboardLayout>
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <Button
              variant="outline"
              onClick={() => router.push('/products/p14')}
          ...
          <Card className="mb-6">
            <div className="p-8">
              ...
              {lesson.day <= 25 && (
                <ActivityCapture
                  activityTypeId={`p14_day_${lesson.day}_activity`}
                  activityName={`Day ${lesson.day} Impact/CSR Activity`}
                  lessonId={lesson.id}
                  courseCode="P14"
                  moduleId={`p14_mod_${Math.ceil(lesson.day / 5)}`}
                />
              )}
            </div>
          </Card>
