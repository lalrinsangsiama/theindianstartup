...
export default function P5LessonPage() {
  ...
  const totalDays = 45;
  const productCode = 'P5';
  const productColor = 'purple';

  useEffect(() => {
    const fetchLesson = async () => {
      try {
        const response = await fetch(`/api/products/p5/lessons/${lessonId}`, {
          credentials: 'include'
        });
        ...
        logger.error('Error fetching P5 lesson:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const completeLesson = async () => {
    ...
    setCompleting(true);
    try {
      const response = await fetch(`/api/products/p5/lessons/${lessonId}/complete`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        if (nextDay <= totalDays) {
          router.push(`/products/p5/lessons/${nextDay}`);
        } else {
          router.push('/products/p5');
        }
      }
    } catch (error) {
      ...
      <DashboardLayout>
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <Button variant="outline" onClick={() => router.push('/products/p5')} className="flex items-center gap-2">
          ...
          <Card className="mb-6">
            <div className="p-8">
              ...
              <Card className="mb-6">
                <div className="p-6">
                  <div className="flex items-center gap-2 mb-4">
                    <Target className={`w-5 h-5 text-${productColor}-600`} />
                    <Heading as="h3" variant="h5">Action Items</Heading>
                  </div>
                  <div className="space-y-3">
                    {lesson.actionItems?.map((item, index) => (
                      <div key={index} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                        <div className={`w-6 h-6 rounded-full bg-${productColor}-100 flex items-center justify-center flex-shrink-0 mt-0.5`}>
                          <Text size="sm" weight="medium" className={`text-${productColor}-700`}>{index + 1}</Text>
                        </div>
                        <Text>{item}</Text>
                      </div>
                    ))}
              ...
              <ActivityCapture
                activityTypeId={`p5_day_${lesson.day}_activity`}
                activityName={`Day ${lesson.day} Legal Activity`}
                lessonId={lesson.id}
                courseCode={productCode}
                moduleId={`p5_mod_${Math.ceil(lesson.day / 4)}`}
              />
            </div>
          </Card>

          <div className="flex items-center justify-between">
            <Button variant="outline" onClick={() => lesson.day > 1 && router.push(`/products/p5/lessons/${lesson.day - 1}`)} disabled={lesson.day <= 1} className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-3">
              {!progress?.completed ? (
                ...
                <Badge variant="success" className="flex items-center gap-2 px-4 py-2">
                  <CheckCircle className="w-4 h-4" /> Completed (+{lesson.xpReward} XP)
                </Badge>
              )}
              <Button onClick={() => lesson.day < totalDays && router.push(`/products/p5/lessons/${lesson.day + 1}`)} disabled={lesson.day >= totalDays} className="flex items-center gap-2">
