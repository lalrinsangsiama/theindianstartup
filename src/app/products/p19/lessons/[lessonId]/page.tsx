...
export default function P19LessonPage() {
  ...
  useEffect(() => {
    const fetchLesson = async () => {
      try {
        const response = await fetch(`/api/products/p19/lessons/${lessonId}`, {
          credentials: 'include'
        });
        ...
        logger.error('Error fetching P19 lesson:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const completeLesson = async () => {
    ...
    setCompleting(true);
    try {
      const response = await fetch(`/api/products/p19/lessons/${lessonId}/complete`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (nextDay <= totalDays) {
          router.push(`/products/p19/lessons/${nextDay}`);
        } else {
          router.push('/products/p19');
        }
      } else {
  ...
  const navigateToDay = (day: number) => {
    router.push(`/products/p19/lessons/${day}`);
  };

  if (loading) {
    return (
      <ProductProtectedRoute productCode="P19">
        <DashboardLayout>
      ...
      <DashboardLayout>
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <Button
              variant="outline"
              onClick={() => router.push('/products/p19')}
          ...
          <Card className="mb-6">
            <div className="p-8">
              ...
              {lesson.day <= 25 && (
                <ActivityCapture
                  activityTypeId={`p19_day_${lesson.day}_activity`}
                  activityName={`Day ${lesson.day} International Expansion Activity`}
                  lessonId={lesson.id}
                  courseCode="P19"
                  moduleId={`p19_mod_${Math.ceil(lesson.day / 5)}`}
                />
              )}
            </div>
          </Card>
