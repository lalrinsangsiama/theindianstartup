...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get P15 product details with modules and lessons
    ...
    if (productError) {
      logger.error('Error fetching P15 product:', productError);
    ...
    if (user) {
      ...
      if (hasAccess && product?.modules) {
        // Get user's progress for P15 lessons
    ...
    logger.error('Error in P15 product API:', error);
