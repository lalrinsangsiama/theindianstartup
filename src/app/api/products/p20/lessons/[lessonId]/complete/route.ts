...
export async function POST(
  ...
  try {
    ...
    // Check if user has access to P20
    const access = await checkProductAccess(user.id, 'P20');
    if (!access.hasAccess) {
      return NextResponse.json(
        { error: 'Access denied. Please purchase P20 course.' },
        { status: 403 }
      );
    }
    ...
    const { data: lesson, error: lessonError } = await supabase
      ...
      `)
      .eq('day', lessonDay)
      .eq('module.product.code', 'P20')
      .single();

    if (lessonError || !lesson) {
      logger.error('Error fetching P20 lesson for completion:', lessonError);
      return NextResponse.json(
    ...
    const { data: purchase, error: purchaseError } = await supabase
      ...
      .or(`productCode.eq.P20,productCode.eq.ALL_ACCESS,productCode.eq.FINTECH_COMPLETE`)
      .limit(1)
      .single();
    ...
    logger.info(`P20 Lesson ${lessonDay} completed by user ${user.id}`);
    ...
    logger.error('Error in P20 lesson completion API:', error);
    return NextResponse.json(
