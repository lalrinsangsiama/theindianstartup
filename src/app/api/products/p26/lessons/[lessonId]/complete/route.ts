...
export async function POST(
  ...
  try {
    ...
    // Check if user has access to P26
    const access = await checkProductAccess(user.id, 'P26');
    if (!access.hasAccess) {
      return NextResponse.json(
        { error: 'Access denied. Please purchase P26 Social Enterprise & Impact Measurement course.' },
        { status: 403 }
      );
    }
    ...
    const { data: lesson, error: lessonError } = await supabase
      ...
      `)
      .eq('day', lessonDay)
      .eq('module.product.code', 'P26')
      .single();

    if (lessonError || !lesson) {
      logger.error('Error fetching P26 lesson for completion:', lessonError);
      return NextResponse.json(
    ...
    const { data: purchase, error: purchaseError } = await supabase
      ...
      .or(`productCode.eq.P26,productCode.eq.ALL_ACCESS`)
      .limit(1)
      .single();
    ...
    logger.info(`P26 Lesson ${lessonDay} completed by user ${user.id}`);
    ...
    logger.error('Error in P26 lesson completion API:', error);
    return NextResponse.json(
