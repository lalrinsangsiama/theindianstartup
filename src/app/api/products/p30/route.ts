...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get P30 product details with modules and lessons
    ...
    if (productError) {
      logger.error('Error fetching P30 product:', productError);
    ...
    if (user) {
      ...
      if (hasAccess && product?.modules) {
        // Get user's progress for P30 lessons
    ...
    logger.error('Error in P30 product API:', error);
