...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get P24 product details with modules and lessons
    ...
    if (productError) {
      logger.error('Error fetching P24 product:', productError);
    ...
    if (user) {
      ...
      if (hasAccess && product?.modules) {
        // Get user's progress for P24 lessons
    ...
    logger.error('Error in P24 product API:', error);
