...
export async function POST(
  ...
  try {
    ...
    // Check if user has access to P29
    const access = await checkProductAccess(user.id, 'P29');
    if (!access.hasAccess) {
      return NextResponse.json(
        { error: 'Access denied. Please purchase P29 Blockchain & Web3 Entrepreneurship course.' },
        { status: 403 }
      );
    }
    ...
    const { data: lesson, error: lessonError } = await supabase
      ...
      `)
      .eq('day', lessonDay)
      .eq('module.product.code', 'P29')
      .single();

    if (lessonError || !lesson) {
      logger.error('Error fetching P29 lesson for completion:', lessonError);
      return NextResponse.json(
    ...
    const { data: purchase, error: purchaseError } = await supabase
      ...
      .or(`productCode.eq.P29,productCode.eq.ALL_ACCESS,productCode.eq.DEEP_TECH`)
      .limit(1)
      .single();
    ...
    logger.info(`P29 Lesson ${lessonDay} completed by user ${user.id}`);
    ...
    logger.error('Error in P29 lesson completion API:', error);
    return NextResponse.json(
