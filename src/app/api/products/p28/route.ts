...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get P28 product details with modules and lessons
    ...
    if (productError) {
      logger.error('Error fetching P28 product:', productError);
    ...
    if (user) {
      ...
      if (hasAccess && product?.modules) {
        // Get user's progress for P28 lessons
    ...
    logger.error('Error in P28 product API:', error);
