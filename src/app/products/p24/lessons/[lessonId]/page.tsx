...
export default function P24LessonPage() {
  ...
  useEffect(() => {
    const fetchLesson = async () => {
      try {
        const response = await fetch(`/api/products/p24/lessons/${lessonId}`, {
          credentials: 'include'
        });
        ...
        logger.error('Error fetching P24 lesson:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const completeLesson = async () => {
    ...
    setCompleting(true);
    try {
      const response = await fetch(`/api/products/p24/lessons/${lessonId}/complete`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (nextDay <= totalDays) {
          router.push(`/products/p24/lessons/${nextDay}`);
        } else {
          router.push('/products/p24');
        }
      } else {
  ...
  const navigateToDay = (day: number) => {
    router.push(`/products/p24/lessons/${day}`);
  };

  if (loading) {
    return (
      <ProductProtectedRoute productCode="P24">
        <DashboardLayout>
      ...
      <DashboardLayout>
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <Button
              variant="outline"
              onClick={() => router.push('/products/p24')}
          ...
          <Card className="mb-6">
            <div className="p-8">
                    ...
                    {lesson.actionItems.map((item, index) => (
                      <div key={index} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                        <div className="w-6 h-6 rounded-full bg-amber-100 flex items-center justify-center flex-shrink-0 mt-0.5">
                          <Text size="sm" weight="medium" className="text-amber-700">
                            {index + 1}
                          </Text>
                        </div>
                        <Text>{item}</Text>
              ...
              {lesson.day <= 25 && (
                <ActivityCapture
                  activityTypeId={`p24_day_${lesson.day}_activity`}
                  activityName={`Day ${lesson.day} Activity`}
                  lessonId={lesson.id}
                  courseCode="P24"
                  moduleId={`p24_mod_${Math.ceil(lesson.day / 5)}`}
                />
              )}
            </div>
          </Card>
