...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get P13 product details with modules and lessons
    ...
    if (productError) {
      logger.error('Error fetching P13 product:', productError);
    ...
    if (user) {
      ...
      if (hasAccess && product?.modules) {
        // Get user's progress for P13 lessons
    ...
    logger.error('Error in P13 product API:', error);
