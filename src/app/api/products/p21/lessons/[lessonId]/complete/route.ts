...
export async function POST(
  ...
  try {
    ...
    // Check if user has access to P21
    const access = await checkProductAccess(user.id, 'P21');
    if (!access.hasAccess) {
      return NextResponse.json(
        { error: 'Access denied. Please purchase P21 course.' },
        { status: 403 }
      );
    }
    ...
    const { data: lesson, error: lessonError } = await supabase
      ...
      `)
      .eq('day', lessonDay)
      .eq('module.product.code', 'P21')
      .single();

    if (lessonError || !lesson) {
      logger.error('Error fetching P21 lesson for completion:', lessonError);
      return NextResponse.json(
    ...
    const { data: purchase, error: purchaseError } = await supabase
      ...
      .or(`productCode.eq.P21,productCode.eq.ALL_ACCESS,productCode.eq.HEALTHTECH_COMPLETE`)
      .limit(1)
      .single();
    ...
    logger.info(`P21 Lesson ${lessonDay} completed by user ${user.id}`);
    ...
    logger.error('Error in P21 lesson completion API:', error);
    return NextResponse.json(
