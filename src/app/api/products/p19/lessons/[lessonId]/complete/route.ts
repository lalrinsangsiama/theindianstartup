...
export async function POST(
  ...
  try {
    ...
    // Check if user has access to P19
    const access = await checkProductAccess(user.id, 'P19');
    if (!access.hasAccess) {
      return NextResponse.json(
        { error: 'Access denied. Please purchase P19 course.' },
        { status: 403 }
      );
    }
    ...
    const { data: lesson, error: lessonError } = await supabase
      ...
      `)
      .eq('day', lessonDay)
      .eq('module.product.code', 'P19')
      .single();

    if (lessonError || !lesson) {
      logger.error('Error fetching P19 lesson for completion:', lessonError);
      return NextResponse.json(
    ...
    const { data: purchase, error: purchaseError } = await supabase
      ...
      .or(`productCode.eq.P19,productCode.eq.ALL_ACCESS,productCode.eq.CORE_FUNCTIONS`)
      .limit(1)
      .single();
    ...
    logger.info(`P19 Lesson ${lessonDay} completed by user ${user.id}`);
    ...
    logger.error('Error in P19 lesson completion API:', error);
    return NextResponse.json(
