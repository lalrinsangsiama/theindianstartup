...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get P26 product details with modules and lessons
    ...
    if (productError) {
      logger.error('Error fetching P26 product:', productError);
    ...
    if (user) {
      ...
      if (hasAccess && product?.modules) {
        // Get user's progress for P26 lessons
    ...
    logger.error('Error in P26 product API:', error);
