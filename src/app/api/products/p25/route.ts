...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get P25 product details with modules and lessons
    ...
    if (productError) {
      logger.error('Error fetching P25 product:', productError);
    ...
    if (user) {
      ...
      if (hasAccess && product?.modules) {
        // Get user's progress for P25 lessons
    ...
    logger.error('Error in P25 product API:', error);
