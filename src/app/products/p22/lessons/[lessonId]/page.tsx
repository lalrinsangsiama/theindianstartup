...
export default function P22LessonPage() {
  ...
  useEffect(() => {
    const fetchLesson = async () => {
      try {
        const response = await fetch(`/api/products/p22/lessons/${lessonId}`, {
          credentials: 'include'
        });
        ...
        logger.error('Error fetching P22 lesson:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const completeLesson = async () => {
    ...
    setCompleting(true);
    try {
      const response = await fetch(`/api/products/p22/lessons/${lessonId}/complete`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (nextDay <= totalDays) {
          router.push(`/products/p22/lessons/${nextDay}`);
        } else {
          router.push('/products/p22');
        }
      } else {
  ...
  const navigateToDay = (day: number) => {
    router.push(`/products/p22/lessons/${day}`);
  };

  if (loading) {
    return (
      <ProductProtectedRoute productCode="P22">
        <DashboardLayout>
      ...
      <DashboardLayout>
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <Button
              variant="outline"
              onClick={() => router.push('/products/p22')}
          ...
          <Card className="mb-6">
            <div className="p-8">
                    ...
                    {lesson.actionItems.map((item, index) => (
                      <div key={index} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                        <div className="w-6 h-6 rounded-full bg-orange-100 flex items-center justify-center flex-shrink-0 mt-0.5">
                          <Text size="sm" weight="medium" className="text-orange-700">
                            {index + 1}
                          </Text>
                        </div>
                        <Text>{item}</Text>
              ...
              {lesson.day <= 25 && (
                <ActivityCapture
                  activityTypeId={`p22_day_${lesson.day}_activity`}
                  activityName={`Day ${lesson.day} Exit Strategy Activity`}
                  lessonId={lesson.id}
                  courseCode="P22"
                  moduleId={`p22_mod_${Math.ceil(lesson.day / 5)}`}
                />
              )}
            </div>
          </Card>
