...
export async function POST(
  ...
  try {
    ...
    // Check if user has access to P23
    const access = await checkProductAccess(user.id, 'P23');
    if (!access.hasAccess) {
      return NextResponse.json(
        { error: 'Access denied. Please purchase P23 Green Entrepreneurship Fundamentals course.' },
        { status: 403 }
      );
    }
    ...
    const { data: lesson, error: lessonError } = await supabase
      ...
      `)
      .eq('day', lessonDay)
      .eq('module.product.code', 'P23')
      .single();

    if (lessonError || !lesson) {
      logger.error('Error fetching P23 lesson for completion:', lessonError);
      return NextResponse.json(
    ...
    const { data: purchase, error: purchaseError } = await supabase
      ...
      .or(`productCode.eq.P23,productCode.eq.ALL_ACCESS,productCode.eq.GREEN_ECONOMY`)
      .limit(1)
      .single();
    ...
    logger.info(`P23 Lesson ${lessonDay} completed by user ${user.id}`);
    ...
    logger.error('Error in P23 lesson completion API:', error);
    return NextResponse.json(
