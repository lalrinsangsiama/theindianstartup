...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get P17 product details with modules and lessons
    ...
    if (productError) {
      logger.error('Error fetching P17 product:', productError);
    ...
    if (user) {
      ...
      if (hasAccess && product?.modules) {
        // Get user's progress for P17 lessons
    ...
    logger.error('Error in P17 product API:', error);
