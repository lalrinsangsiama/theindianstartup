...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get P16 product details with modules and lessons
    ...
    if (productError) {
      logger.error('Error fetching P16 product:', productError);
    ...
    if (user) {
      ...
      if (hasAccess && product?.modules) {
        // Get user's progress for P16 lessons
    ...
    logger.error('Error in P16 product API:', error);
