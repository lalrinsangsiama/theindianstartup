...
export async function POST(
  ...
  try {
    ...
    // Check if user has access to P15
    const access = await checkProductAccess(user.id, 'P15');
    if (!access.hasAccess) {
      return NextResponse.json(
        { error: 'Access denied. Please purchase P15 Carbon Credits & Sustainability course.' },
        { status: 403 }
      );
    }
    ...
    const { data: lesson, error: lessonError } = await supabase
      ...
      `)
      .eq('day', lessonDay)
      .eq('module.product.code', 'P15')
      .single();

    if (lessonError || !lesson) {
      logger.error('Error fetching P15 lesson for completion:', lessonError);
      return NextResponse.json(
    ...
    const { data: purchase, error: purchaseError } = await supabase
      ...
      .or(`productCode.eq.P15,productCode.eq.ALL_ACCESS,productCode.eq.SECTOR_MASTERY`)
      .limit(1)
      .single();
    ...
    logger.info(`P15 Lesson ${lessonDay} completed by user ${user.id}`);
    ...
    logger.error('Error in P15 lesson completion API:', error);
    return NextResponse.json(
