...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get P21 product details with modules and lessons
    ...
    if (productError) {
      logger.error('Error fetching P21 product:', productError);
    ...
    if (user) {
      ...
      if (hasAccess && product?.modules) {
        // Get user's progress for P21 lessons
    ...
    logger.error('Error in P21 product API:', error);
