...
export default function P1LessonPage() {
  ...
  useEffect(() => {
    const fetchLesson = async () => {
      try {
        const response = await fetch(`/api/products/p1/lessons/${lessonId}`, {
          credentials: 'include'
        });
        ...
        logger.error('Error fetching P1 lesson:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const completeLesson = async () => {
    ...
    setCompleting(true);
    try {
      const response = await fetch(`/api/products/p1/lessons/${lessonId}/complete`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (nextDay <= totalDays) {
          router.push(`/products/p1/lessons/${nextDay}`);
        } else {
  ...
  const navigateToDay = (day: number) => {
    router.push(`/products/p1/lessons/${day}`);
  };
      ...
      <DashboardLayout>
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <Button
              variant="outline"
              onClick={() => router.push('/products/p1')}
          ...
          <Card className="mb-6">
            <div className="p-8">
              ...
              <ActivityCapture
                activityTypeId={`p1_day_${lesson.day}_activity`}
                activityName={`Day ${lesson.day} Launch Sprint Activity`}
                lessonId={lesson.id}
                courseCode="P1"
                moduleId={`p1_mod_${Math.ceil(lesson.day / 7)}`}
              />
            </div>
          </Card>
