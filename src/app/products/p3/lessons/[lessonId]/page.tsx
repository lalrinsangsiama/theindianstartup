...
export default function P3LessonPage() {
  ...
  useEffect(() => {
    const fetchLesson = async () => {
      try {
        const response = await fetch(`/api/products/p3/lessons/${lessonId}`, {
          credentials: 'include'
        });
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (nextDay <= totalDays) {
          router.push(`/products/p3/lessons/${nextDay}`);
        } else {
          router.push('/products/p3');
        }
      } else {
  ...
  const navigateToDay = (day: number) => {
    router.push(`/products/p3/lessons/${day}`);
  };
      ...
      <DashboardLayout>
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <Button
              variant="outline"
              onClick={() => router.push('/products/p3')}
          ...
          <Card className="mb-6">
            <div className="p-8">
              ...
              <ActivityCapture
                activityTypeId={`p3_day_${lesson.day}_activity`}
                activityName={`Day ${lesson.day} Funding Activity`}
                lessonId={lesson.id}
                courseCode="P3"
                moduleId={`p3_mod_${Math.ceil(lesson.day / 4)}`}
              />
            </div>
          </Card>
