...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get P20 product details with modules and lessons
    ...
    if (productError) {
      logger.error('Error fetching P20 product:', productError);
    ...
    if (user) {
      ...
      if (hasAccess && product?.modules) {
        // Get user's progress for P20 lessons
    ...
    logger.error('Error in P20 product API:', error);
