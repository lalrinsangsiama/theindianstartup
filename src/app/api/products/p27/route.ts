...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get P27 product details with modules and lessons
    ...
    if (productError) {
      logger.error('Error fetching P27 product:', productError);
    ...
    if (user) {
      ...
      if (hasAccess && product?.modules) {
        // Get user's progress for P27 lessons
    ...
    logger.error('Error in P27 product API:', error);
