...
export async function POST(
  ...
  try {
    ...
    // Check if user has access to P14
    const access = await checkProductAccess(user.id, 'P14');
    if (!access.hasAccess) {
      return NextResponse.json(
        { error: 'Access denied. Please purchase P14 Impact & CSR Mastery course.' },
        { status: 403 }
      );
    }
    ...
    const { data: lesson, error: lessonError } = await supabase
      ...
      `)
      .eq('day', lessonDay)
      .eq('module.product.code', 'P14')
      .single();

    if (lessonError || !lesson) {
      logger.error('Error fetching P14 lesson for completion:', lessonError);
      return NextResponse.json(
    ...
    const { data: purchase, error: purchaseError } = await supabase
      ...
      .or(`productCode.eq.P14,productCode.eq.ALL_ACCESS,productCode.eq.SECTOR_MASTERY`)
      .limit(1)
      .single();
    ...
    logger.info(`P14 Lesson ${lessonDay} completed by user ${user.id}`);
    ...
    logger.error('Error in P14 lesson completion API:', error);
    return NextResponse.json(
