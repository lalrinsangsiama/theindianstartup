...
export async function POST(
  ...
  try {
    ...
    // Check if user has access to P18
    const access = await checkProductAccess(user.id, 'P18');
    if (!access.hasAccess) {
      return NextResponse.json(
        { error: 'Access denied. Please purchase P18 course.' },
        { status: 403 }
      );
    }
    ...
    const { data: lesson, error: lessonError } = await supabase
      ...
      `)
      .eq('day', lessonDay)
      .eq('module.product.code', 'P18')
      .single();

    if (lessonError || !lesson) {
      logger.error('Error fetching P18 lesson for completion:', lessonError);
      return NextResponse.json(
    ...
    const { data: purchase, error: purchaseError } = await supabase
      ...
      .or(`productCode.eq.P18,productCode.eq.ALL_ACCESS,productCode.eq.CORE_FUNCTIONS`)
      .limit(1)
      .single();
    ...
    logger.info(`P18 Lesson ${lessonDay} completed by user ${user.id}`);
    ...
    logger.error('Error in P18 lesson completion API:', error);
    return NextResponse.json(
