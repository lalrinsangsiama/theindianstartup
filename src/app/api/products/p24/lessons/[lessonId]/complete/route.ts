...
export async function POST(
  ...
  try {
    ...
    // Check if user has access to P24
    const access = await checkProductAccess(user.id, 'P24');
    if (!access.hasAccess) {
      return NextResponse.json(
        { error: 'Access denied. Please purchase P24 Circular Economy Business Models course.' },
        { status: 403 }
      );
    }
    ...
    const { data: lesson, error: lessonError } = await supabase
      ...
      `)
      .eq('day', lessonDay)
      .eq('module.product.code', 'P24')
      .single();

    if (lessonError || !lesson) {
      logger.error('Error fetching P24 lesson for completion:', lessonError);
      return NextResponse.json(
    ...
    const { data: purchase, error: purchaseError } = await supabase
      ...
      .or(`productCode.eq.P24,productCode.eq.ALL_ACCESS,productCode.eq.GREEN_ECONOMY`)
      .limit(1)
      .single();
    ...
    logger.info(`P24 Lesson ${lessonDay} completed by user ${user.id}`);
    ...
    logger.error('Error in P24 lesson completion API:', error);
    return NextResponse.json(
