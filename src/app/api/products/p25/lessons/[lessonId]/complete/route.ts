...
export async function POST(
  ...
  try {
    ...
    // Check if user has access to P25
    const access = await checkProductAccess(user.id, 'P25');
    if (!access.hasAccess) {
      return NextResponse.json(
        { error: 'Access denied. Please purchase P25 Rural & Agri-Tech Innovation course.' },
        { status: 403 }
      );
    }
    ...
    const { data: lesson, error: lessonError } = await supabase
      ...
      `)
      .eq('day', lessonDay)
      .eq('module.product.code', 'P25')
      .single();

    if (lessonError || !lesson) {
      logger.error('Error fetching P25 lesson for completion:', lessonError);
      return NextResponse.json(
    ...
    const { data: purchase, error: purchaseError } = await supabase
      ...
      .or(`productCode.eq.P25,productCode.eq.ALL_ACCESS`)
      .limit(1)
      .single();
    ...
    logger.info(`P25 Lesson ${lessonDay} completed by user ${user.id}`);
    ...
    logger.error('Error in P25 lesson completion API:', error);
    return NextResponse.json(
