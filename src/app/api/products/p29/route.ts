...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get P29 product details with modules and lessons
    ...
    if (productError) {
      logger.error('Error fetching P29 product:', productError);
    ...
    if (user) {
      ...
      if (hasAccess && product?.modules) {
        // Get user's progress for P29 lessons
    ...
    logger.error('Error in P29 product API:', error);
