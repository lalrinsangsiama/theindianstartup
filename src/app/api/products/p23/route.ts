...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get P23 product details with modules and lessons
    ...
    if (productError) {
      logger.error('Error fetching P23 product:', productError);
    ...
    if (user) {
      ...
      if (hasAccess && product?.modules) {
        // Get user's progress for P23 lessons
    ...
    logger.error('Error in P23 product API:', error);
