...
export async function POST(
  ...
  try {
    ...
    // Check if user has access to P17
    const access = await checkProductAccess(user.id, 'P17');
    if (!access.hasAccess) {
      return NextResponse.json(
        { error: 'Access denied. Please purchase P17 course.' },
        { status: 403 }
      );
    }
    ...
    const { data: lesson, error: lessonError } = await supabase
      ...
      `)
      .eq('day', lessonDay)
      .eq('module.product.code', 'P17')
      .single();

    if (lessonError || !lesson) {
      logger.error('Error fetching P17 lesson for completion:', lessonError);
      return NextResponse.json(
    ...
    const { data: purchase, error: purchaseError } = await supabase
      ...
      .or(`productCode.eq.P17,productCode.eq.ALL_ACCESS,productCode.eq.CORE_FUNCTIONS`)
      .limit(1)
      .single();
    ...
    logger.info(`P17 Lesson ${lessonDay} completed by user ${user.id}`);
    ...
    logger.error('Error in P17 lesson completion API:', error);
    return NextResponse.json(
